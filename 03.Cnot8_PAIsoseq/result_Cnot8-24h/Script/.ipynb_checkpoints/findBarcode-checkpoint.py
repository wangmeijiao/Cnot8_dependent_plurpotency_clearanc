...
def cleanCCS(transcript, mismatch, count):
	...
	for it in matches: #barcode+6nt match, only hit once?
		...
		p5Adapter = '(AAGCAGTGGTATCAACGCAGAGTAC){e<=2}([ATCG]{9,11})(ATGGG)'
		p5_matches = regex.finditer(p5Adapter, seq, overlapped = False)
		for p5_it in p5_matches: #5adaptor match, only hit once?
		...
		if(clean_seq != ""):
			...
			out = [target_name, read_id, barcode_name, p5_tso,UMI,atggg ,strand, clean_seq, clean_qual] #for degug only
			out = '\t'.join([str(x) for x in out])
			print(out)
		else:
			sys.stderr.write( target_name + '\tempty\n') #add keyword empty by mjwang
